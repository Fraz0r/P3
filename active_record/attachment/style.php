...
//- Public
	...
	public function resize($input, $output, $size)
	{
		...
		$to_size = $this->_get_size_options($image, $size);
...
//- Private
	/**
	 * This is used by resize().  It calls _parse_size_string to get the width
	 ...
	 */
	private function _get_size_options($image, $size_str)
	{
		$crop    = false;
		$best_fit = false;

		$to_size = $this->_parse_size_string($size_str);
	...
	/**
	 * This is the starting step for _get_size_options().  It takes the size string,
	 ...
	 */
	private function _parse_size_string($str)
	{
		$ret = array();
