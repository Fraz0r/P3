...
class Attachment extends \P3\Model\Base
{
	...
	private $_options = array(
		'path' => ':p3_root/public/system/:class/:attachment/:id/:style_:basename.:extension',
...
//- Public
	...
	public function path($style = null)
	{
		...
		return $this->_parse_template_string($template, $style);
	}
	...
	public function reprocess()
	{
		...
		return $this->_generate_styles() && $this->delete_junk();
	}
	...
	public function save($file_data)
	{
		...
		switch($data['error'])
		{
			...
			case \UPLOAD_ERR_FORM_SIZE:
				...
				$this->_parent->errors->add($field, 'Maximum File Size of '.$val.' Exceeded');
				return false;
			case \UPLOAD_ERR_PARTIAL:
				$this->_parent->errors->add($field, 'Only part of the file was uploaded');
				return false;
			case \UPLOAD_ERR_NO_TMP_DIR:
				$this->_parent->errors->add($field, 'There is no temporary directory configured for your PHP Installation');
				return false;
			case \UPLOAD_ERR_CANT_WRITE:
				$this->_parent->errors->add($field, 'PHP cannot write to configured temporary directory');
				return false;
			case \UPLOAD_ERR_EXTENSION:
				$this->_parent->errors->add($field, 'A PHP extension stopped the file upload');
				return false;
			default:
				$this->_parent->errors->add($field, 'An unknown error happened during the file upload');
				return false;
		}
...
//- Private
	 ...
	 * @return boolean successfullness 
	 */
	private function _generate_styles()
	{
		$flag = true;

		foreach($this->_options['styles'] as $style_name => $size)
			$flag = $flag && $this->_get_style_for($size)->resize($this->path(), $this->path($style_name), $size);
	 ...
	 */
	private function _get_style_for($style_str = null, array $options = array())
	{
		return new Attachment\Style($this->_interpret_style($style_str), $options);
	}
	 ...
	 */
	private function _interpret_style($style_str = null)
	{
	 ...
	 */
	private function _parse_template_string($str, $style = null)
	{
		if(is_null($style))
		...
		$replace = array(
			':p3_root'    => \P3\ROOT,
			':class'      => \str::from_camel(get_class($this->_parent)),
			':attachment' => $this->_name,
			':id'         => $this->_parent->id(),
			':style'      => $style,
			':basename'   => $info->get_filename_no_extension(),
			':extension'  => $info->get_extension()
		);
